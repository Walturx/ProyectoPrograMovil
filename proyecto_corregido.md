...
| **Flujo principal** | 1. El huésped accede a "Recompensas". 2. El sistema lista las recompensas activas (`rewards`, `is_active = 1`). 3. El huésped selecciona una recompensa y confirma el canje. 4. Se crea un registro en `reward_redemptions` y se resta de `loyalty_accounts.available_stars`. |
