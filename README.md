...
| **Flujo principal** | 1. Se muestra el resumen (hotel, habitación, fechas, total). 2. El huésped ingresa datos de tarjeta. 3. Se registra el pago en `payments` con estado `approved`. 4. La reserva pasa a `confirmed`. 5. Se genera un QR y se suman estrellas a `loyalty_accounts`. |
| **Tablas** | `reservations`, `payments`, `loyalty_accounts`, `loyalty_transactions` |
...
| **Flujo principal** | 1. El huésped accede a la tienda de estrellas. 2. El sistema lista recompensas activas (`rewards`, `is_active = 1`). 3. Selecciona una recompensa y confirma. 4. Se crea un registro en `reward_redemptions` y se descuenta de `loyalty_accounts.available_stars`. |
| **Tablas** | `rewards`, `reward_redemptions`, `loyalty_accounts` |
